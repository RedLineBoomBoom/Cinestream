...
import { formatServerName, getServerBadgeInfo, getDefaultServer, isAnimeMedia, isIndonesianMedia } from '../../utils/formatters';
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-sans font-medium border ${getServerBadgeInfo(activeServer, language, isAnime, isIndo).color}`}>
                {getServerBadgeInfo(activeServer, language, isAnime, isIndo).label} • {activeServer.quality}
              </span>
            </div>
    ...
    <div className="bg-[#181818]/90 border border-white/10 rounded-xl p-4 sm:p-5 backdrop-blur-xl shadow-xl shadow-black/40">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
        {servers.map((server, idx) => {
          ...
          const badge = getServerBadgeInfo(server, language, isAnime, isIndo);
          return (
            <button
              key={server.id}
              onClick={() => {
      ...
      <div className="mt-3 pt-2.5 border-t border-white/[0.06] flex items-center gap-2 text-[11px] text-slate-400 font-normal">
        ...
        <span>
          {isIndo
            ? t('guideIndoServerNotice')
            : isAnime
            ? t('guideAnimeServerNotice')
            : t('guideServerNotice')}
        </span>
      </div>
    </div>
